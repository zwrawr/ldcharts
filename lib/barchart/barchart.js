...
export default class BarChart extends Component {
	scaleValues( values, minZero, posMin, posMax ) {
		...
		return { valuesYPos, yZeroPos, yOnePos };
	}
	...
	render( props ) {
		...
		const usePercentages = (props.usePercentages === undefined) ? false : props.usePercentages;
		...
		if ( props.showYAxis ) {
			ShowYAxis = (
				<YAxis
					yZeroPos={yZeroPos} yOnePos={yOnePos}
					xZeroPos={xZeroPos} width={yAxisWidth}
					padTop={padTop} padBottom={padBottom}
					showTicks={props.showYTicks}
				/>
			);
		}
		...
		values = values.map((v) => Math.round(100*v)/100);
		...
		let ShowLegend = (labels) ? <Legend names={Names} colors={Colors} /> :null;
