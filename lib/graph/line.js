...
export default class Line extends Component {
	...
	render( props, state ) {
		...
		let lineclass = classNames("-line", "vis_stroke_color_"+color, props.class);
		let pointclass = classNames("-point", "vis_fill_color_"+color, props.class);
		let textclass = classNames("-text -hidden", "vis_fill_color_"+color, props.class);
		...
		for (let i = 0; i < x.length; i++) {
			...
			if (sy[i] && sx[i]){
				...
				const title = "(" + x[i] + "," + y[i] + ")";
				points.push(<circle cx={sx[i]} cy={sy[i]} class={pointclass} r="1"  />);
